"""v5.2 — same as v5 but compresses C4-lookup descriptions to train.csv style.

train.csv descriptions are short paraphrases (median ~200-300 chars, 1-2
sentences). Raw C4 finding bodies are technical prose that often runs longer
and ends mid-sentence after a 350-char cut. v5.2 keeps only the first 1-2
complete sentences (up to ~280 chars) of each C4 body, ending at sentence
boundary. Should push the BGE similarity of the c4_lookup-sourced 296 rows
higher against the truth set.
"""
import csv
import json
import os
...
OUT = "outputs/submission_c4_v5_2.csv"


def compress_to_sentences(text: str, target_len: int = 280) -> str:
    """Take first complete sentences up to target_len chars. Avoids mid-sentence cuts."""
    text = text.strip()
    ...
    for s in sents:
        ...
        if not out:
            out = s
            continue
        if len(out) + 1 + len(s) > target_len:
            break
        out += " " + s
    return out if out else text[:target_len]
...
def main():
    ...
    for repo in test_repos:
        ...
        for f in findings:
            ...
            best = int(sims.argmax())
            ref = labeled[best]
            desc = compress_to_sentences(clean_body(f["body"], platform), target_len=280)
            if len(desc) < 30:
