...
DEFAULT_MAX_FINDINGS_PER_REPO = 5
...
def parse_args():
    ...
    parser.add_argument("--output", default="outputs/submission_v2.csv")
...
def build_train_summary(train_rows, max_findings_per_repo):
    ...
    for row in train_rows:
        ...
        if description:
            description_bank[(tag, subtag, severity)].append(description)
    ...
    prior_pool = []
    for (tag, subtag, severity), count in combo_counts.most_common(30):
        descriptions = description_bank[(tag, subtag, severity)]
        representative = descriptions[0] if descriptions else make_description(tag, subtag)
        prior_pool.append(
            {
                "tag": tag,
                "subtag": subtag,
                "severity": severity,
                "support": count,
                "description": representative if len(representative) <= 360 else make_description(tag, subtag),
            }
        )
...
def choose_target_count(repo_id, repo_size_samples, max_findings_per_repo):
    if not repo_size_samples:
        return min(4, max_findings_per_repo)
    index = stable_hash(repo_id) % len(repo_size_samples)
    return max(2, min(repo_size_samples[index], max_findings_per_repo))
...
def collect_prior_candidates(repo_id, prior_pool):
    candidates = []
    if not prior_pool:
        return candidates

    offset = stable_hash(repo_id) % len(prior_pool)
    window = min(len(prior_pool), 18)
    for index in range(window):
        item = prior_pool[(offset + index) % len(prior_pool)]
        score = 0.25 + math.log1p(item["support"]) * 0.08 - index * 0.003
        candidates.append(
            {
                "tag": item["tag"],
                "subtag": item["subtag"],
                "severity": item["severity"],
                "description": make_description(item["tag"], item["subtag"]),
                "score": score,
                "source": "prior",
            }
        )
...
def build_submission_rows(args, train_summary, test_repo_ids, train_text_by_repo, train_fingerprints, test_text_by_repo, test_fingerprints):
    ...
    for repo_id in test_repo_ids:
        ...
        if args.mode in {"prior", "hybrid"} or not candidate_pool:
            candidate_pool.extend(collect_prior_candidates(repo_id, train_summary["prior_pool"]))

        ranked = rank_candidates(merge_candidates(candidate_pool))
        picked = ranked[:target_count]
...
def main():
    ...
    train_summary = build_train_summary(train_rows, args.max_findings_per_repo)
    ...
    print("Baseline v2 submission generated successfully.")
