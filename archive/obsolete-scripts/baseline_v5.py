...
DEFAULT_MAX_FINDINGS_PER_REPO = 15
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Bastet baseline v5: diverse per-repo descriptions to maximize BGE similarity."
    )
...
def build_train_summary(train_rows, max_findings_per_repo, prior_pool_size, high_severity_pool_size):
    ...
    def make_prior_item(combo):
        ...
        # Store all unique descriptions sorted by quality (80-420 chars first, then by length desc)
        unique_descs = list(dict.fromkeys(descriptions))
        good = [d for d in unique_descs if 80 <= len(d) <= 420]
        rest = [d for d in unique_descs if d not in set(good)]
        sorted_descs = sorted(good, key=lambda d: -len(d)) + rest
        all_descriptions = sorted_descs if sorted_descs else [make_description(tag, subtag)]
        return {
            "tag": tag,
            "subtag": subtag,
            "severity": severity,
            ...
            "description": all_descriptions[0],  # keep for backward compat
            "descriptions": all_descriptions,     # full pool for diversity
        }
        ...
        candidates.append(
            {
                ...
                "descriptions": item.get("descriptions", [item["description"]]),  # pass full pool
                "score": score,
                "source": source,
            }
        )
...
def build_submission_rows(args, train_summary, test_repo_ids, train_text_by_repo, train_fingerprints, test_text_by_repo, test_fingerprints):
    ...
    for repo_id in test_repo_ids:
        ...
        for candidate in picked:
            # Pick a repo-specific description from the pool to maximize diversity
            desc_pool = candidate.get("descriptions", [candidate["description"]])
            desc_idx = stable_hash(repo_id + candidate["tag"] + candidate["subtag"]) % len(desc_pool)
            desc = desc_pool[desc_idx]
            all_rows.append(
                {
                    "Property": 0,
                    "repo_path": repo_id,
...
def main():
    ...
    print("Baseline v5 submission generated successfully.")
